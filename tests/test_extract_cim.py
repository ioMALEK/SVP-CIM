...
# ------------------------------------------------------------------ #
def test_make_seed():
    s1 = make_seed("tag", 123)
    s2 = make_seed("tag", 123)
    s3 = make_seed("tag", 456)
    assert s1 == s2 != s3
    assert 0 <= s1 < 2 ** 32
    assert s1 != GLOBAL_SEED
...
def test_tqdm_joblib():
    total = 8
    with tqdm_joblib(tqdm(total=total, disable=True)):
        out = Parallel(n_jobs=2)(
            delayed(lambda x: x * x)(i) for i in range(total)
        )
    assert out[-1] == (total - 1) ** 2
